...
task mosdepth {
  ...
  command <<<
    ...
    if [ ! -f ~{sample_id}..mosdepth.summary.txt ]; then
      mv --verbose ~{out_prefix}.mosdepth.summary.txt ~{sample_id}.mosdepth.summary.txt
      mv --verbose ~{out_prefix}.regions.bed.gz ~{sample_id}.regions.bed.gz
      mv --verbose ~{out_prefix}.regions.bed.gz.csi ~{sample_id}.regions.bed.gz.csi
    fi
    ...
    import pandas as pd
    df = pd.read_csv('~{sample_id}.mosdepth.summary.txt', sep='\t')
  ...
  output {
    ...
    File   region_bed_index        = "~{sample_id}.regions.bed.gz.csi"
  ...
  runtime {
    docker: "~{runtime_attributes.container_registry}/mosdepth:0.3.9_build1"
    cpu: threads
    memory: mem_gb + " GiB"
    disk: disk_size + " GB"
