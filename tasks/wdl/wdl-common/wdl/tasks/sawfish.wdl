...
task sawfish_call {
  ...
  command <<<
    ...
    | bcftools view - \
      --threads ~{(threads / 2) - 1} \
      --output-type z \
      --output ~{out_prefix}.structural_variants.vcf.gz
    bcftools index --tbi \
      ~{if threads > 1 then "--threads " + (threads - 1) else ""} \
      ~{out_prefix}.structural_variants.vcf.gz
  ...
  output {
    File  vcf                              = "~{out_prefix}.structural_variants.vcf.gz"
    File  vcf_index                        = "~{out_prefix}.structural_variants.vcf.gz.tbi"
